...
//        self.configUI()
    ...
    func loadData() {
        ...
        if self.view.tag == 2 {
            group.enter()
            self.viewModel.getFollwerData(user_id: userId, offset: "\(self.offset)", onSuccess: { (success) in
            ...
            group.enter()
            self.viewModel.getFollowingData(user_id: userId, offset: "\(self.offset)", onSuccess: { (success) in
                if !success {
                    self.isFound = false
            ...
            group.enter()
            self.viewModel.getReviewDeta(user_id: userId, offset: "\(self.offset)", onSuccess: { (success) in
                if !success {
                    self.isFound = false
    ...
    @objc func followButtonAct(_ sender: UIButton) {
        if (UserDefaultModule.shared.getUserData()?.user_id ?? "" != "") {
            ...
            if type == "follow" {
                ...
                sender.backgroundColor = UIColor(named: "AppThemeColor")
            }
            else {
