...
class ItemDetailsMapTableViewCell: UITableViewCell {
    ...
    var isFirst = true // Load Map image first time Only
    var bgimage = ""
    ...
    override func awakeFromNib() {
     ...
     func configUI() {
        ...
        self.callButton.backgroundColor = UIColor(named: "AppThemeColor") ?? .white
    ...
    func loadData(_ itemDetails: ItemModel) {
        ...
        if Addons_Status{
            ...
            if isFirst {
                let GooglemapKey = ADMIN_VIEW_MODEL.adminModel?.result.googlemapkey_ios ?? ""
                let url = "\(MAP_URL)\(itemDetails.latitude ?? 0),\(itemDetails.longitude ?? 0)" + "&zoom=15&size=\(Int(self.contentView.frame.width))x\(Int(self.mapImageView.frame.height-25))&maptype=roadmap&key=\(GooglemapKey)&sensor=false.jpg"
                print("GOOGLEADMINMAP:\(GooglemapKey)")
                isFirst = false
                print("mapURL \(url)")
