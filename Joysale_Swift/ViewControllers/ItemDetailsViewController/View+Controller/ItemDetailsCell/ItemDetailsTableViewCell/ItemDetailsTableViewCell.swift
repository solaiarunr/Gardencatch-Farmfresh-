...
//                     self.adButton.setTitle(item.promotionType, for: .normal)
                     ...
                     if item.membership_enable == "enable"{
                         ...
                         self.adButtonWidth.constant = 100
                     }else{
                         self.memberShipView.isHidden = true
                     }
                     self.adButton.isHidden = true
                ...
                self.adButton.isHidden = true
                self.memberShipLabel.isHidden = true
                self.memberShipView.isHidden = true
             }
         }
 ...
 //        }
     ...
     func loadOrderData(_ item: MyOrderResultModel, viewType: Int) {
         ...
         if item.status == "cancelled" {
             ...
             self.productConditionButton.backgroundColor = UIColor(named: "appblackcolor")
         }
         else if item.status == "processing" {
             self.productConditionButton.setTitle(getLanguage["Under Processing"] ?? item.status, for: .normal)
             self.productConditionButton.setBackgroundColor(color: UIColor(named: "appblackcolor") ?? .white, forState: .normal)
         }
