 ...
 class ChatSubTableViewCell: UITableViewCell {
    ...
    var viewModel: ChatListModel?
     ...
     func configUI() {
         ...
         self.dateLabel.config(color: UIColor(named: "AppTextColor"), font: UIFont(name: APP_FONT_REGULAR, size: 13), align: .right, text: "")
         self.timeLabel.config(color: UIColor(named: "AppTextColor"), font: UIFont(name: APP_FONT_REGULAR, size: 13), align: .left, text: "")
         self.imageTimeLabel.config(color: UIColor(named: "whitecolor"), font: UIFont(name: APP_FONT_REGULAR, size: 13), align: .left, text: "")
         ...
         self.declineButton.config(color: UIColor(named: "AppTextColor"), font: UIFont(name: APP_FONT_REGULAR, size: 15), align: .center, title: "decline")
...
//        self.languageButton.config(color: UIColor(named: "AppThemeColor"), font: UIFont(name: APP_FONT_REGULAR, size: 14), align: .right, title: "")
     ...
     func loadOfferValue() {
         if self.offerStatusLabel.text?.contains("\(getLanguage["about"] ?? "")") ?? false {
             self.offerStatusLabel.textColor = UIColor(named: "AppThemeColor") ?? .white
        ...
        self.languageButton.isHidden = true
         ...
         if chatModel.type == "about" {
            ...
            self.wholeStackView.alignment = .fill
             ...
             if chatModel.offerType == "sendreceive" {
                 ...
                 if chatModel.sender == (UserDefaultModule.shared.getUserData()?.userName ?? "") && chatModel.instantBuy == 1 && chatModel.buynowStatus == 0 && BUYNOW_MODEL_FLAG {
                     self.dateLabel.config(color: UIColor(named: "whitecolor"), font: UIFont(name: APP_FONT_REGULAR, size: 13), align: .left, text: "")
                     self.dateLabel.text = self.timeLabel.text!
            ...
            self.chatImageView.contentMode = .scaleAspectFill
             ...
             self.imageChatView.isHidden = false
             self.chatView.isHidden = true
             let urlString = (chatModel.message.uploadImage ?? "")
             let urlStringEncoded = urlString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
             var imageURL = urlStringEncoded
...
//             }
  ...
  */
  //          let url = "https://maps.google.com/maps/api/staticmap?markers=color:red|\(chatModel.message.latitude ?? ""),\(chatModel.message.longitude ?? "")" + "&zoom=15&size=\(Int(self.chatImageView.frame.width))x\(Int(self.chatImageView.frame.height))&maptype=roadmap&key=\(GOOGLE_API_KEY)&sensor=false.jpg"
             let url = "https://maps.google.com/maps/api/staticmap?markers=color:red|\(chatModel.message.latitude ?? ""),\(chatModel.message.longitude ?? "")" + "&zoom=15&size=\(Int(self.chatImageView.frame.width))x\(Int(self.chatImageView.frame.height))&maptype=roadmap&key=\(ADMIN_VIEW_MODEL.adminModel?.result.googlemapkey_ios ?? "")&sensor=false.jpg"
         ...
         if chatModel.type == "offer" {
             ...
             self.rightView.isHidden = false
            self.languageButton.isHidden = true
              self.chatView.backgroundColor = UIColor(named: "green_color")
             self.rightImageView.tintColor = UIColor(named: "green_color")
             self.imageChatView.backgroundColor = UIColor(named: "green_color")
        ...
        if chatModel.type == "send" {
            self.messageLabel.textColor = .white
            self.dateLabel.textColor = .white
            self.timeLabel.textColor = .white
        } else if chatModel.type == "recieve"{
            self.messageLabel.textColor = .black
            self.dateLabel.textColor = .black
            self.timeLabel.textColor = .black
        }
