 ...
 class ChatViewController: UIViewController {
    ...
    @IBOutlet var chatBg: UIImageView!
...
//    var smartReplyArray = [SmartReplySuggestion]()
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = chat_bg
//        self.sendWholeView.backgroundColor = chat_bg
//        textView.backgroundColor = .white
//        sendStackView.backgroundColor = .white
        sendWholeView.layer.cornerRadius = 10;
        sendWholeView.layer.masksToBounds = true;
        //sendWholeView.frame = CGRect(x: 20, y: 597, width: FULL_HEIGHT-20, height: 60)
...
//        self.timer.invalidate()
    ...
    func NavigationBarButtonItem() {
        ...
        button2.titleLabel?.font = UIFont(name: APP_FONT_REGULAR, size: 17) ?? UIFont.systemFont(ofSize: 14)
    ...
    func rightBarButtonItem() {
        ...
        //change option button image
        rbutton.setImage(UIImage(named: "new_option.png"), for: .normal)
...
//        }
    ...
    func loadData() {
        ...
        self.viewModel.getChatData(sender_id:
                                    (UserDefaultModule.shared.getUserData()?.user_id ?? ""), receiver_id: self.receiverId, type: "normal", source_id: "\(self.itemDetails?.id ?? 0)", offset: "0", limit: "20", onSuccess: { (success) in
                                        if success {
                                            if self.offSet == 0 {
        ...
        ADMIN_VIEW_MODEL.getProfileData(user_id: self.receiverId, user_name: "", onSuccess: { (success) in
            self.NavigationBarButtonItem()
  ...
  */
            ...
            if self.textView.tag != 0 && self.textView.text != ""{
                self.sendChat(message: self.textView.text!, current_latitude: "", current_longitude: "", image_url: "", source_id: self.isFromItemDetails ? "\(self.itemDetails?.id ?? 0)" : "", type: "normal", viewUrl: "", message_content: "1")
            }
            else {
    ...
    func sendChat(message: String,  current_latitude: String, current_longitude: String, image_url: String, source_id: String, type: String, viewUrl: String, audioDuration: String = "", message_content: String = "1") {
        ...
        let chatModel = ChildChatModel(type: typeVal, receiver: (ADMIN_VIEW_MODEL.profileModel?.result.userName ?? ""), sender: (UserDefaultModule.shared.getUserData()?.userName ?? ""), message: messageVal, sourceID: source_id, itemImage: self.itemDetails?.photos.first?.itemUrlMain350 ?? "", itemTitle: self.itemDetails?.itemTitle ?? "", audioDuration: audioDuration)
...
//                cell.progressSlider.clipsToBounds = true
        ...
        */
        /*
        else if self.chatModelArray[indexPath.row].type == "audio" || self.chatModelArray[indexPath.row].type == "video" {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if self.chatModelArray.count > indexPath.row {
            ...
            if chatModel.type == "share_location" {
                ...
                let ChatImgURL = chatModel.message.uploadImage ?? ""
                print("ChatImgURL -->\(ChatImgURL)")
                let encodeURL = ChatImgURL.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
//                let escapedString = originalString.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)
                imageSource.append(SDWebImageSource(urlString: encodeURL ?? "", placeholder: #imageLiteral(resourceName: "profilelogo"))!)
        ...
        if scrollView == self.tableView && self.lastContentOffset > scrollView.contentOffset.y {
            ...
            if contentOffset <= 50 && !self.isLoadingMore {
                ...
                DispatchQueue.global(qos: .background).async {
                    ...
                    self.viewModel.getChatData(sender_id: (UserDefaultModule.shared.getUserData()?.user_id ?? ""), receiver_id: self.receiverId, type: "normal", source_id: "\(self.itemDetails?.id ?? 0)", offset: "\(self.offSet)", limit: "20", onSuccess: { (success) in
                        if success {
...
//        if collectionView == self.collectionView {
            ...
            cell.shadowView.backgroundColor = UIColor(named: "whitecolor")
            cell.filterLabel.textColor = UIColor(named: "AppThemeColor")
            cell.filterLabel.text = getLanguage[(ADMIN_VIEW_MODEL.adminModel?.result.chatTemplate[indexPath.row].name ?? "")] ?? (ADMIN_VIEW_MODEL.adminModel?.result.chatTemplate[indexPath.row].name ?? "")
            cell.cancelButton.isHidden = true
            return cell
//        }
...
//        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == self.collectionView {
            self.sendChat(message: getLanguage[(ADMIN_VIEW_MODEL.adminModel?.result.chatTemplate[indexPath.row].name ?? "")] ?? (ADMIN_VIEW_MODEL.adminModel?.result.chatTemplate[indexPath.row].name ?? ""), current_latitude: "", current_longitude: "", image_url: "", source_id: self.isFromItemDetails ? "\(self.itemDetails?.id ?? 0)" : "", type: "normal", viewUrl: "", message_content: "1")
        }
        else {
