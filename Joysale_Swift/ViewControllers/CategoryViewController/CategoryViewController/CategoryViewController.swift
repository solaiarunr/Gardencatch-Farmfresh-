 ...
 */
    ...
    func loadCategoryIndex() {
        if let result = ADMIN_VIEW_MODEL.adminModel?.result {
            if (self.CategoryDetails.Category_id ?? "0") == "" {
                self.CategoryDetails.Category_id = "\(result.category.first?.categoryId ?? 0)"
            }
            self.categoryIndex = result.category.firstIndex(where: {$0.categoryId == (Int(self.CategoryDetails.Category_id ?? "0"))}) ?? 0
            if result.category.count > self.categoryIndex {
                self.subCategory =  result.category[self.categoryIndex].subcategory
            }
    ...
    @IBAction func textFieldEditingChanged(_ sender: UITextField) {
        ...
        if let category = ADMIN_VIEW_MODEL.adminModel?.result.category {
...
extension CategoryViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return (ADMIN_VIEW_MODEL.adminModel?.result.category.count ?? 0)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let category = ADMIN_VIEW_MODEL.adminModel?.result.category[indexPath.row] {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if self.categoryViewType == 0 {
            if (ADMIN_VIEW_MODEL.adminModel?.result.category.count ?? 0) % 2 == 1 {
                if indexPath.row == ((ADMIN_VIEW_MODEL.adminModel?.result.category.count ?? 0) - 1) {
                    return CGSize(width: (collectionView.frame.width), height: size.height)
                }
                else {
                    return CGSize(width: (collectionView.frame.width / 2), height: size.height)
                }
            }
            else {
                return CGSize(width: (collectionView.frame.width / 2), height: size.height)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        self.CategoryDetails.Category_id = "\(ADMIN_VIEW_MODEL.adminModel?.result.category[indexPath.row].categoryId ?? 0)"
        self.CategoryDetails.subcategory_id = ""
        self.CategoryDetails.child_category_id = ""

            if (ADMIN_VIEW_MODEL.adminModel?.result.category[indexPath.row].subcategory.count ?? 0) > 0 {
                if self.categoryViewType == 0 {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 && self.isFromFilter{
            ...
            if self.CategoryDetails.subcategory_id == "viewall" {
                cell.checkImageView.isHidden = false
                cell.viewAllButton.setTitleColor(UIColor(named: "AppThemeColor"), for: .normal)
            }
            else {
                cell.viewAllButton.setTitleColor(UIColor(named: "AppTextColor"), for: .normal)
            }
            ...
            if indexPath.row == 0 && isFromFilter{
                ...
                cell.searchLabel.textColor = UIColor(named: "AppTextColor")
                if self.isSearchFlag {
