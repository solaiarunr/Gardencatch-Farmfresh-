        ...
        if loginType == "Apple" {
            ...
            CallParsingFunction().postDataCall(subURl: DELETE_ACCOUNT, params: parameter, onSuccess: { (response) in
                ...
                if rootClass.status == true {
                    ...
                    let messageAlert = UIAlertController(title: getLanguage["alert"], message: rootClass.message, preferredStyle: .alert)
                    messageAlert.addAction(UIAlertAction(title: getLanguage["ok"], style: .default, handler: { (UIAlertAction) in
...
//            self.delegate.navigationController.pushViewController(pageObj, animated: true)
            ...
            pageObj.viewType = "donate"
            pageObj.donateContent = self.viewModel.profileModel?.result.donateDetails.pageContent ?? ""
//            pageObj.helpResult = self.viewModel.helpModel?.result[indexPath.section]
        ...
        else if self.profileArr[indexPath.section] == "deleteaccount"{
