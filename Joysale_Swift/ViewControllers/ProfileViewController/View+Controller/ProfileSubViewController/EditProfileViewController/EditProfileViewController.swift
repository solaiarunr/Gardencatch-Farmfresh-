...
class EditProfileViewController: UIViewController, customLocationDelegate {
    ...
    func loadData() {
        self.viewModel.getProfileData(user_id: UserDefaultModule.shared.getUserData()?.user_id ?? "", user_name: "", onSuccess: { (success) in
            print(success)
            if success {
        ...
        if error == nil {
            ...
            do {
                ...
                if (self.profileData?.mobileNo ?? "") != "\(phoneNumbers.countryCode)\(phoneNumbers.nationalNumber)" {
                    ...
                    print("MOBILDATA:\(self.profileData?.mobileNo ?? "")")
...
extension EditProfileViewController {
  ...
  private func handleFacebookAuthentication() {
    ...
    loginManager.logIn(permissions: ["email", "public_profile"], from: self) { (result, error) in
      ...
      GraphRequest(graphPath: "me", parameters: ["fields": "id,name,first_name,last_name,email,birthday,gender,location,hometown,about,likes,work,education,picture,photos"]).start(completion: { (connection, result, error) -> Void in
