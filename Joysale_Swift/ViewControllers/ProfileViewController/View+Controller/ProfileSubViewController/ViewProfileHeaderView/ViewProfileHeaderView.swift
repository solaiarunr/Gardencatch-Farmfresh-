...
class ViewProfileHeaderView: UIView {
    ...
    func customUI() {
        ...
        self.userNameLabel.config(color: UIColor(named: "whitecolor"), font: UIFont(name: APP_FONT_BOLD, size: 20), align: .left, text: "")
    ...
    func loadData(_ profileData: ProfileResultModel) {
        ...
        if profileData.membership_enable == "enable"{
            self.memberShipIcon.isHidden = false
        }else{
            self.memberShipIcon.isHidden = true
        }
