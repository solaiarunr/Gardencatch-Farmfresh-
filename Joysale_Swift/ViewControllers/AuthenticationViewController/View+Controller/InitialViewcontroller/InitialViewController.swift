...
class InitialViewController: UIViewController {
    ...
    @IBOutlet weak var privacyEnableBtn: UIButton!
    ...
    func configUI() {
        ...
        let mainString = getLanguage["by continuing, agree our Terms of use & Privacy policy"] ?? "by continuing, agree our Terms of use & Privacy policy"
        let subString = getLanguage["Privacy Policy"] ?? "Privacy Policy"
        let range = (mainString as NSString).range(of: subString)
        let mutableAttributedString = NSMutableAttributedString(string: mainString , attributes: [NSAttributedString.Key.font: UIFont(name: APP_FONT_REGULAR, size: 16)!])
        mutableAttributedString.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor(named: "AppThemeColor")!, range: range)
        mutableAttributedString.addAttribute(NSAttributedString.Key.font, value: UIFont(name: APP_FONT_BOLD, size: 16)!, range: range)
        /*
        let subString1 = getLanguage["Terms of use"] ?? "Terms of use"
        let range1 = (mainString as NSString).range(of: subString1)
        mutableAttributedString.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor(named: "AppThemeColor")!, range: range1)
        mutableAttributedString.addAttribute(NSAttributedString.Key.font, value: UIFont(name: APP_FONT_BOLD, size: 16)!, range: range1)
        */
        self.privacyLbl.addGestureRecognizer(UITapGestureRecognizer(target:self, action: #selector(termsLabelAct(gesture:))))
        self.privacyLbl.isUserInteractionEnabled = true
        self.privacyLbl.attributedText = mutableAttributedString

    }
    @IBAction func termsLabelAct(gesture: UITapGestureRecognizer) {
        let policyRange = (self.privacyLbl.text! as NSString).range(of: (getLanguage["Privacy Policy"] ?? "Privacy Policy"))
        if gesture.didTapAttributedTextInLabel(label: self.privacyLbl, inRange: policyRange) {
            print("Working")
            let pageObj = HelpViewController()
            pageObj.viewType = "privacy_policy"
    ...
    @IBAction func privacyEnableButtonAct(_ sender: UIButton){
        if sender.tag == 0 {
            self.privacyEnableBtn.tag = 1
            self.privacyEnableBtn.setImage(UIImage(named: "Check_sel"), for: .normal)
            self.privacyEnableBtn.tintColor = UIColor(named: "AppThemeColor")
        }
        else {
            self.privacyEnableBtn.tag = 0
            self.privacyEnableBtn.setImage(UIImage(named: "Check_Unsel"), for: .normal)
        }
    }
    
    @IBAction func facebookButtonAct(_ sender: UIButton) {
        if self.privacyEnableBtn.tag == 1 {
            self.handleFacebookAuthentication()
        }else{
            let alert = UIAlertController(title: "", message: (getLanguage["Accept our terms & Conditions"] ?? "Accept our terms & Conditions"), preferredStyle: .alert)
            let action = UIAlertAction(title: getLanguage["OK"] ?? "OK", style: .cancel) { (UIAlertAction) in
            }
...
extension InitialViewController: ASAuthorizationControllerDelegate {
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            let client_id = BUNDLE_NAME
            print("Bundle ID \(client_id)")
            let idToken = appleIDCredential.identityToken!
            let client_secret = String(data: idToken, encoding: .utf8)
            print("JSON web token \(client_secret!)")
            let token = appleIDCredential.authorizationCode
            let encodedToken = String(data: token!, encoding: .utf8)
            print("Token access \(encodedToken)")
            let token_type_hint = "access_token"
            print("token hint \(token_type_hint)")
            print("pop:\(userData)")
            let userDefaults = UserDefaultModule.shared
            userDefaults.setType(key: "Apple")
            userDefaults.setClientId(key: client_id)
            userDefaults.setClientSecret(key: client_secret!)
            userDefaults.setAccesssToken(key: encodedToken!)
            userDefaults.setTokenType(key: token_type_hint)
            if appleUserEmail != "" {
