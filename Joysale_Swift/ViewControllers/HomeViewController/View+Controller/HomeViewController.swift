...
class HomeViewController: UIViewController, customLocationDelegate {
    ...
    @IBOutlet weak var Camera_Btn_View: UIView!
   ...
   // var bannerView1:GADBannerView!
    ...
    override func viewDidLoad() {
        ...
        self.Camera_Btn_View.isHidden = true
...
//        self.locationLabel.text = FILTER_DATA.location == "" ? "Worldwide" : FILTER_DATA.location
        ...
        if self.filterArray.count > 0 {
            ...
            if (FILTER_DATA.distance != "" && FILTER_DATA.isDistanceSlider && FILTER_DATA.location.lowercased() != CURRENT_LOCATION.lowercased()) {
            }
            else {
...
//        self.present(obj, animated: true, completion: nil)
    ...
    func configUI() {
        //Sell Image Label Details
        self.Camera_Btn_View.layer.cornerRadius = 25
        self.Camera_Btn_View.layer.masksToBounds = true
        ...
        self.locationLabel.config(color: UIColor(named: "ThemeTextColor"), font: UIFont(name: APP_FONT_REGULAR, size: 13), align: .center, text: "WorldWide")
 ...
 */
    ...
    func loadData() {
        ...
        if self.filterArray.count == 0 && (FILTER_DATA.location == "" || FILTER_DATA.location.lowercased() == "worldwide" || FILTER_DATA.location.lowercased() == CURRENT_LOCATION.lowercased()) {
            self.search_type = "all"
            FILTER_DATA.lat = ""
            FILTER_DATA.long = ""
            if self.offset == 0 {
        ...
        else if (FILTER_DATA.location == "" || FILTER_DATA.location.lowercased() == "worldwide" || FILTER_DATA.location.lowercased() == CURRENT_LOCATION.lowercased()) {
            FILTER_DATA.lat = ""
            FILTER_DATA.long = ""
        }
        ...
        self.viewModel.getItems(type: "search", price: FILTER_DATA.Price, search_key: FILTER_DATA.Search_key, category_id: FILTER_DATA.Category_id, subcategory_id: FILTER_DATA.subcategory_id, user_id: UserDefaultModule.shared.getUserData()?.user_id ?? "", item_id: FILTER_DATA.item_id, seller_id: FILTER_DATA.seller_id, sorting_id: FILTER_DATA.getSortID(), offset: "\(self.offset)", limit: "50", posted_within: FILTER_DATA.posted_within, distance: FILTER_DATA.distance, distance_type: FILTER_DATA.distance_type, lang_type: DEFAULT_LANGUAGE_CODE, filters: filterString, product_condition: "\(product_id)", child_category_id: FILTER_DATA.child_category_id, lon: FILTER_DATA.long, lat: FILTER_DATA.lat,search_type: search_type,promotionTags: self.promotionTags, onSuccess: { (success) in
            if self.offset == 0 {
    ...
    func startTimer() {
        ...
        timer = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(self.timerAct), userInfo: nil, repeats: true)
    }
    func stopTimer() {
        timer?.invalidate()
        timer = nil
    ...
    @objc func timerAct() {
        print("hiiii \(ADMIN_VIEW_MODEL.adminModel?.result.bannerData.count)")
...
extension HomeViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
        ...
        if collectionView == self.collectionView {
            if indexPath.section == 0 {
                ...
                if FILTER_DATA.getSortID() == "2"{
                    if self.itemModel.count > indexPath.row && self.itemModel.count >= 50 {
                    ...
                    if self.itemModel.count > indexPath.row && self.itemModel.count >= 20 {
                        if indexPath.row == (self.itemModel.count - 1) && self.isFound {
                            self.offset = self.itemModel.count
                            self.bottomLoader.startAnimating()
                            self.loadData()
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == self.collectionView && indexPath.section == 2 {
            if self.itemModel.count > indexPath.row {
