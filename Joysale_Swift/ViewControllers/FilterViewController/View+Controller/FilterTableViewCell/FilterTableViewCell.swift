 ...
 class FilterTableViewCell: UITableViewCell {
     ...
     func configUI() {
         ...
         self.maximumPriceLabel.config(color: UIColor(named: "AppThemeColor"), font: UIFont(name: APP_FONT_REGULAR, size: 15), align: .right, text: "")
         self.filterDescLabel.config(color: UIColor(named: "AppTextColor"), font: UIFont(name: APP_FONT_REGULAR, size: 15), align: .right, text: "")
         self.minimumPriceLabel.config(color: UIColor(named: "AppThemeColor"), font: UIFont(name: APP_FONT_REGULAR, size: 15), align: .left, text: "")
...
//                 self.filterLabel.text = getLanguage["WorldWide"]
                self.filterLabel.text = CURRENT_LOCATION
             }
             ...
             print("DISTANCE:\(FILTER_DATA.distance ?? "")")
             ...
             if ((Int(FILTER_DATA.distance) ?? 0) != 0 && FILTER_DATA.location != "" && FILTER_DATA.location.lowercased() != "WorldWide".lowercased()) || (FILTER_DATA.location != "" && FILTER_DATA.location.lowercased() != "WorldWide".lowercased() && !FILTER_DATA.isDistanceSlider) {
                 if (FILTER_DATA.location != "" && FILTER_DATA.location.lowercased() != "WorldWide".lowercased() && !FILTER_DATA.isDistanceSlider && FILTER_DATA.location.lowercased() != CURRENT_LOCATION.lowercased()) {
...
//                    FILTER_DATA.distance = ""
                ...
                print("DISTANCETYPE :\(FILTER_DATA.distance_type ?? "")")
                if FILTER_DATA.distance == ""{
                    self.distanceButton.isHidden = true
                }else{
                    self.distanceButton.setTitle("\(FILTER_DATA.distance ?? "") \(FILTER_DATA.distance_type ?? "")", for: .normal)
                 }
                 ...
                 self.homeFilterImageView.tintColor = UIColor(named: "AppThemeColor")
                 self.destinationFilterImageView.tintColor = UIColor(named: "AppThemeColor")
             }
             else {
