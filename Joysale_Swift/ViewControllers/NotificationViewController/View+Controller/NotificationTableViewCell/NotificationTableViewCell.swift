    ...
    func loadData(_ result: NotificationResultModel) {
        ...
        if result.type == "admin" {
            ...
            if result.message.uppercased() == "CONTACTED YOU ON YOUR PRODUCT" {
                ...
                else if result.message.contains("declined your offer request".lowercased()){
                    content = (getLanguage["declined_your_exchange_request_on"] ?? "") + "\(result.itemTitle.html2String)"
                }
                else {
