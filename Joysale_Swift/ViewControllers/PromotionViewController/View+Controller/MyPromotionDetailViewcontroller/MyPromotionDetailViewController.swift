...
class MyPromotionDetailViewController: UIViewController {
    ...
    func loadData() {
        if isFromItemDetails {
            self.promotionModel?.itemImage = self.itemDetails?.photos.first?.itemUrlMainOriginal ?? ""
            self.promotionModel?.itemName = self.itemDetails?.itemTitle ?? ""
            self.collectionView.reloadData()
            self.NavigationBarButtonItem()
            self.tableView.isHidden = true
            Utility.shared.startAnimation(viewController: self)
            self.viewModel.checkPromotionData(item_id: "\(self.itemDetails?.id ?? 0)", onSuccess: { (success) in
