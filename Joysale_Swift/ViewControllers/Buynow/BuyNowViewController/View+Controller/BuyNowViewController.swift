...
class BuyNowViewController: UIViewController {
    ...
    func configUI() {
        ...
        self.checkOutButton.backgroundColor = UIColor(named: "AppThemeColor") ?? .white
...
//        }
    ...
    @objc func presentStripe() {
        let currencyCode = self.itemDetails?.currencyCode.components(separatedBy: "-")
        var cSymbol = ""
        if (currencyCode?.count ?? 0) > 1 {
            cSymbol = currencyCode?[1] ?? ""
        }
        let shippingCost = self.itemDetails?.shippingCost ?? ""
        var price = "\(self.itemDetails?.price ?? "0")"
        if offerPayment {
            price = offerPrice
        }
