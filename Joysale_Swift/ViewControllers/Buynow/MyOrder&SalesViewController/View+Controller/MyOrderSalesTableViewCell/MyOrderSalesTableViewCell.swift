...
class MyOrderSalesTableViewCell: UITableViewCell {
    ...
    func configUI() {
        ...
        self.listNameLabel.config(color: UIColor(named: "appblackcolor"), font: UIFont(name: APP_FONT_BOLD, size: 15), align: .left, text: "")
        self.statusButton.cornerMiniumRadius()
        self.statusButton.config(color: UIColor(named: "whitecolor"), font: UIFont(name: APP_FONT_REGULAR, size: 15), align: .center, title: "")
    }
    func loadData(_ orderData: MyOrderResultModel, viewType: Int) {
        ...
        if orderData.status == "cancelled" {
            ...
            self.statusButton.setBackgroundColor(color: UIColor(named: "appblackcolor") ?? .white, forState: .normal)
        }
            ...
            self.statusButton.setBackgroundColor(color: UIColor(named: "AppThemeColor") ?? .white, forState: .normal)
        }
        else if orderData.status == "claimed" {
            self.statusButton.setTitle(getLanguage["Claimed"] ?? orderData.status, for: .normal)
            self.statusButton.backgroundColor = UIColor(named: "appblackcolor")
            self.statusButton.setBackgroundColor(color: UIColor(named: "appblackcolor") ?? .white, forState: .normal)
        }
