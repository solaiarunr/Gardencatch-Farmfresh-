...
class AddressListViewController: UIViewController {
    ...
    func configUI() {
        ...
        self.addAddressButton.backgroundColor = UIColor(named: "AppThemeColor") ?? .white
    ...
    func loadData() {
        self.viewModel.getShippingAddressAct(user_id: (UserDefaultModule.shared.getUserData()?.user_id ?? ""), item_id: "\(self.itemDetails?.id ?? 0)", onSuccess: { (success) in
