...
//                if let countryID = ADMIN_VIEW_MODEL.productBeforeModel?.result.country.lastIndex(where: {$0.countryName == "OTHERS"}) {
//                    self.countryArray.remove(at: countryID)
                    //print(countryID\"dsdff,jkbckbkbkbksjbvlsbdjvbsdjlbvs")
                }
    ...
    @IBAction func continueButtonAct(_ sender: UIButton) {
        ...
        let addresstwo = self.addressModelArray.filter({$0.title == "addresstwo"}).first?.value ?? ""
         ...
         let country = self.addressModelArray.filter({$0.title == "country"}).first?.value ?? ""
...
extension AddressViewController: UITableViewDelegate, UITableViewDataSource, addressDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        self.addressModelArray[indexPath.section].value = CURRENT_LOCATION
        cell.countryTextField.isUserInteractionEnabled = false
         */
        return cell
    }
    ...
    @objc func configDropDownView(_ sender: UIButton) {
        ...
        if sender.tag == 0 {
            ...
            dropDown.selectionAction = { [unowned self] (index: Int, item: String) in
               ...
               // self.addressModelArray[9].value = "\(ADMIN_VIEW_MODEL.productBeforeModel?.result.country[index].countryId ?? 0)"
               // let currencyID = self.countryArray.filter({$0.countryName == country}).first?.countryId ?? 0
