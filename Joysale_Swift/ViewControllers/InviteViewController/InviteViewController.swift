...
//        let escapedString = INVITE_URL.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)
        let escapedString = "https://apps.apple.com/us/app/Farm%20Fresh%2024/7/id1078268802"
        let newurl = "https://gardencatch.com/buy"

        content.contentURL = URL(string: newurl)!
...
//        let inviteURL = "\(getLanguage["Invite from"] ?? "") \(APP_NAME)".addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) ?? ""
//        let titleURL = "<h4>\(getLanguage["invite_friend"] ?? "") \(APP_NAME): <a href='\(INVITE_URL)'>\(INVITE_URL_2)</h4>".html2String.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) ?? ""
        
        let titleURL = "\(getLanguage["Invite from"] ?? "") \(APP_NAME)".addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) ?? ""
        let inviteURL = "<h4>\(getLanguage["invite_friend"] ?? "") \(APP_NAME): <a href='\(INVITE_URL)'>\(INVITE_URL_2)</h4>".html2String.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) ?? ""
...
//            let message = "<h4>\(getLanguage["invite_friend"] ?? "") \(APP_NAME): <a href='\(INVITE_URL)'>\(BASE_URL)</h4>"
//            mail.setMessageBody(message, isHTML: true)
            mail.setSubject("\(getLanguage["Invite from"] ?? "") \(APP_NAME)")
            let message = "<h4>\(getLanguage["invite_friend"] ?? "") \(APP_NAME): <a href='\(INVITE_URL)'>\(INVITE_URL_2)</h4>"
            mail.setMessageBody(message, isHTML: true)
    ...
    @IBAction func whatsAppShareAct(_ sender: UIButton) {
        let url = "whatsapp://send?text=\(getLanguage["invite_friend"] ?? "") \(INVITE_URL_2)"
...
extension InviteViewController: SharingDelegate {
    ...
    func sharer(_ sharer: Sharing, didFailWithError error: Error) {
        ...
        print("FbMessanger didFailWithError")
    }
