...
//        self.priceTextField.config(color: UIColor(named: "AppTextColor"), align: .left, placeHolder: "type_your_offer", font: UIFont(name: APP_FONT_REGULAR, size: 15))
        ...
        self.userNameLabel.text = (itemDetails?.sellerName ?? "")
...
//        self.view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.dismissViewAct)))
    ...
    @IBAction func sendButtonAct(_ sender: UIButton) {
        ...
        else if (Float(self.priceTextField.text!) ?? 0) > (Float(self.itemDetails?.price ?? "0") ?? 0) {
    ...
    func sendChatAfterID(_ chatID: String) {
        self.viewModel.sendofferRequest(sender_id: (UserDefaultModule.shared.getUserData()?.user_id ?? ""), source_id: "\(self.itemDetails?.id ?? 0)", chat_id: chatID, message: self.descTextView.text!, offer_price: self.priceTextField.text!, onSuccess: { (success) in
