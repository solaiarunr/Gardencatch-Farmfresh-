...
//        self.isPresentAds = false
        self.setStatusBarBackgroundColor(color: UIColor(named: "BlackColor") ?? .black)
...
//                fatalError("No video device found")
    ...
    @IBAction func nextButtonAct(_ sender: UIButton) {
        if self.selectedImageArray.count <= 5 && self.selectedImageArray.count > 0 {
            if isTabBar {
